...
if_record = input("Do you want to video save your experience (Y,N)🧐 : ").upper()
...
if if_record == "Y":
    ...
    video_path = "./video_records/" + video_name
    print("Video {} has been started recording.🤗".format(video_name))
    ...
    print("This Memory is NOT been Preserved.😔")
...
# Creating Object for Face Deduction.
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor("face_landmarks/shape_predictor_68_face_landmarks.dat")

while True:
    _, frameFlipped = cap.read()
    frames = cv2.flip(frameFlipped, 1) # Flipping to get Mirror Image.
    ...
    for face in faces:
        ...
        # For Reference, Points image in landmarks_image/landmarks_68_points.png.
    ...
    key = cv2.waitKey(1)
    if key == 32:
        # Loop Enters, when Space bar is Pressed for Clicking Image.
        time_stamp = datetime.now().strftime("%d-%M-%Y_%I'%M'%S")
        image_name = "piggy-nose-image_{}.png".format(time_stamp)
        image_path = "./image_records/" + image_name
